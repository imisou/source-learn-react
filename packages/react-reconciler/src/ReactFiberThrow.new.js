...
function throwException(
  root: FiberRoot,
  returnFiber: Fiber,    // 父节点 FiberNode
  sourceFiber: Fiber,    // 正在处理的 FiberNode
  value: mixed,
  rootRenderLanes: Lanes,
) {
  // The source fiber did not complete.
  // 将节点标记 Incomplete 未完成 副作用
  ...
  // 1. 处理 Lazy 组件通过 throw 抛出异常的方式去决定是否显示 fallback
  if (
    value !== null &&
    typeof value === 'object' &&
  ...
  // 向上寻找捕获异常的组件
  //  1. 根节点
  //  2. Class组件 且 定义了 getDerivedStateFromError | componentDidCatch
  do {
    switch (workInProgress.tag) {
      ...
      case ClassComponent:
        ...
        // 判断组件是否存在 DidCapture 副作用，且 定义了 getDerivedStateFromError | componentDidCatch
        if (
