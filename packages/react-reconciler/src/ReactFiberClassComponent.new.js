...
/**
 * 主要两个作用：
 * 1. 给组件实例对象添加 setState, forceUpdate 等函数的实现方法
 * 2. 将Class组件的 instace 和 fiberNode 关联起来
 *   - fiber.stateNode = instance
 *   - instance._reactInternalFiber = workInProgress;
 */
function adoptClassInstance(workInProgress: Fiber, instance: any): void {
  // 给组件实例对象添加 setState , forceUpdate 等函数的实现方法
  instance.updater = classComponentUpdater;
  // 将Class组件的实例对象赋值给 workInProgress.stateNode
  workInProgress.stateNode = instance;
  // The instance needs access to the fiber so that it can schedule updates
  // 将instance._reactInternalFiber = workInProgress;
...
/**
 * 初始化Class组件的 constructor
 *  1. 获取 Class组件的 context 上下文对象
 *  2. 根据 props context 初始化执行 constructor 函数
 *  3. 给组件实例对象添加 setState, forceUpdate 等函数的实现方法
 *  4. 将Class组件的 instace 和 fiberNode 关联起来
 *   - fiber.stateNode = instance
 *   - instance._reactInternalFiber = workInProgress;
 */
...
/**
 *  回调 Class 组件的 componentWillMount 生命周期函数
 *   根据新旧state判断是否产生 ReplaceState 类型的更新
 */
function callComponentWillMount(workInProgress, instance) {
  ...
  // 如果新旧state不相等，说明产生了 ReplaceState 类型的更新
...
function mountClassInstance(
  ...
  instance.refs = {};
  // 初始化 Class组件的 updateQueue 对象
  initializeUpdateQueue(workInProgress);
  // 获取 Class 组件的上下文 context 对象
  const contextType = ctor.contextType;
  if (typeof contextType === 'object' && contextType !== null) {
  ...
  // 根据constructor 构造函数返回的对象 去赋值初始化的 state
  instance.state = workInProgress.memoizedState;
  //  1. 回调 getDerivedStateFromProps 生命周期函数
  ...
  // 如果存在 componentDidMount 生命周期函数，设置 Update 标记
...
function resumeMountClassInstance(
  ...
  // 直接获取 instance 对象, 所以不需要执行 constructor 构造函数
  const instance = workInProgress.stateNode;
